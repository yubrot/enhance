...
- [x] **Commit 3: Sort plan node and executor** — `SortItem`, `QueryPlan::Sort` variant with EXPLAIN support. `Sort<C>` executor node. Planner logic for ORDER BY: alias resolution (check SELECT aliases first), positional references (`ORDER BY 1`), aggregate context rewrite.
  ...
  - Deviation: Commits 3 and 4 were combined into a single implementation pass because Sort and Limit are tightly coupled in `plan.rs`, `runner.rs`, and `planner.rs` (LIMIT/OFFSET depends on the same plan-tree structure that ORDER BY produces). NaN ordering in sort is not explicitly tested (handled by `Value::partial_cmp` + `unwrap_or(Equal)` fallback).

- [x] **Commit 4: Limit plan node and executor** — `QueryPlan::Limit` variant (u64 fields) with EXPLAIN support. `Limit<C>` executor node. Planner logic: bind LIMIT/OFFSET as constant expressions, evaluate to i64, validate type (integer) and sign (non-negative) at planning time, convert to u64.
